...
First, we intend to extend existing methods for functions that depend on the state at a single time in the future~\cite{skulj2015}, which fail when the time interval grows large. 
...
\item
We should argue for its practical relevance (for example, for which type of applications would the things we want to try and compute be relevant).
\item
