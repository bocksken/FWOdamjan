...
The results of such inferences can then be used in various applications and situations, for instance to make predictions that are robust against modelling errors. For the discrete time case, examples of such applications can be found in a wide range of domains, including optical character recognition~\cite{debock2014:estihmm}, gesture recognition~\cite{soullard2017:gesturerecognition}, ecology \cite{samuels2009phd} and finance~\cite{niemiec2007:bonusmalus}. For the continuous-time case, however, we are aware of only two preliminary applications, in telecommunication networks~\cite{rottondi2017:flexigrid} and power system analysis~\cite{Troffaes+GSB-ISIPTA15p}, respectively. The aim of this research project is to develop theoretical results and computational methods for this continous-time case, thereby making it possible to tackle real-life large-scale applications as well.
