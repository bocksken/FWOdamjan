...
{\bf\color{blue} (DAMJAN WRITES A FIRST DRAFT)} 

Markov chains are among most popular models in probability theory. They describe the evolution of various systems in time. Such a system can be in one of the predefined states  and then at certain time it moves to another state. At what time and to which state it will move next is the matter of probabilistic modelling. How likely some move is, depends on the state where the system is now, and not on the states where it has been before. It is popular to say that such processes have no memory. 

When the model is set-up, it is surprisingly convenient to make the predictions about its future behaviour. However, to make reliable predictions, one would need to have perfect knowledge of the parameters that rule the process. But with the exception of some purely theoretical models, this is almost never possible. The longer the process evolves, the less reliable are the predictions made on the basis of unreliable parameters.  To make our predictions more reliable we have to sacrifice their precision. This is possible by applying the models of imprecise probabilities. 

The project aims at generalizing the theory of Markov chains in continuous time to imprecise probabilities. This continues the very successful path of modelling imprecise discrete time Markov chains. The majority of the work on imprecise Markov models has been done at the Universities of Ghent and Ljubljana. We now intend to formalize this long time cooperation. 
...
For each of the following, we need to provide the title, first name, surname, date of birth, current occupation, employment (\%), e-mail, institution, research unit, street and number, postal code, city and country
\vspace{2pt}
...
First name: Gert\\
Surname: De Cooman\\
Date of birth: {\bf\color{blue}???}\\
Current occupation: Professor\\
Employment (\%): 100\\
E-mail: gert.decooman@ugent.be\\
Institution: Ghent University\\
Research unit: Imprecise Probability unit of the Data Science Lab, Department of Electronics and Information Systems, Faculty of Engineering and Architecture\\
...
{\color{blue}\bf *** A detailed list of all publications of the supervisor appointed at a Flemish host institution and all other (co-)supervisors should be submitted through the FWO E-portal before the final submission date of the call!
...
I would choose the following: {\color{blue}\bf (What do you guys think?)}\\
- Probability theory;\\
- Knowlege representation and reasoning.

Other related options in the list were:\\
...
My suggestions (will get contact details for those that we propose):
...
{\color{blue}\bf Should we mention the projects of Stavros and Alexander?
