...
Markov chains are among the most popular probabilistic models, and they are very successful at describing the uncertain evolution of various systems in time. 
Such a Markov chain can be in any one of several states, and moves between these states as time progresses. 
At each time instant and in each state, it is uncertain which state it will move to next, and this uncertainty is modelled using so-called transition probabilities, which are assumed to depend only on the current state. 

Mathematically, it turns out to be surprisingly convenient to make predictions about the future behaviour of a Markov chain, and this makes it a popular tool in various applied domains, including engineering, artificial intelligence, mathematical finance and bio-informatics. However, in order to make these predictions reliable, perfect knowledge of the parameters---the transition probabilities---of the Markov chain is required, and such knowledge is almost never realistically present. 
...
The aim of this project is to formalise this mutual interest, and to use our combined expertise to develop a theory and toolbox for dealing with imprecise Markov chains in continuous time, thereby making it possible to apply imprecise Markov chains in real-life problems, the majority of which involve continuous time.
